  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result?.error) {
        ...
        router.push('/dashboard'); // o altra pagina post-login
      }
    } catch (err) {
    ...
    <div className="flex items-center justify-center min-h-screen bg-gray-100 dark:bg-gray-900">
      <Card className="w-full max-w-md dark:bg-gray-800 dark:border-gray-700">
        ...
        <CardFooter className="text-sm text-center dark:text-gray-400">
          Non hai un account?{' '}
          <a href="/auth/register" className="font-medium text-blue-600 hover:underline dark:text-sky-400 dark:hover:text-sky-300">
            Registrati
          </a>
        </CardFooter>
      </Card>
