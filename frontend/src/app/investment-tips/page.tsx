...
export default function InvestmentTipsPage() {
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      {error && <p className="text-red-500 text-center mb-4">Errore: {error}</p>}
      {noResults && !loading && <p className="text-gray-600 text-center mb-4">Nessun consiglio trovato con i criteri specificati.</p>}
      {loading && <p className="text-gray-600 text-center mb-4">Caricamento...</p>}

      {results.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {results.map((tip) => (
            <div key={tip.id} className="bg-white shadow-lg rounded-lg overflow-hidden p-6 flex flex-col justify-between">
              <div>
                <h2 className="text-xl font-semibold mb-2 text-indigo-700">{tip.title}</h2>
                {tip.description && <p className="text-gray-700 text-sm mb-2">{tip.description}</p>}
                {tip.content && <p className="text-gray-600 text-sm mb-3 whitespace-pre-wrap">{tip.content.substring(0,150)}...</p>}
                {tip.category && <p className="text-sm text-gray-500 mb-1"><strong>Categoria:</strong> {tip.category.name}</p>}
                {tip.riskLevel && <p className="text-sm text-gray-500 mb-1"><strong>Rischio:</strong> {tip.riskLevel}</p>}
                {tip.expectedReturn && <p className="text-sm text-gray-500 mb-1"><strong>Rendimento Atteso:</strong> {tip.expectedReturn}%</p>}
                {tip.author && <p className="text-xs text-gray-400 mt-3"><em>Autore: {tip.author.name}</em></p>}
              </div>
              {sessionStatus === 'authenticated' && (
                <button
                  onClick={() => toggleFavorite(tip.id)}
                  className={`mt-4 font-bold py-2 px-4 rounded w-full transition duration-150 ease-in-out 
                    ${tip.isFavorite 
